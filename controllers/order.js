...
sgMail.setApiKey('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');
...
exports.orderById = (req, res, next, id) => {
    Order.findById(id)
    .populate('products.prodact', 'name price')
    .exec((err, order) => {
         if(err || !order) {
...
exports.create = async (req, res) => {
   const geoData = await geocoder.forwardGeocode({
        query: req.body.order.address,
        limit: 1
    }).send()
    req.body.order.user = req.profile;
    const order = new Order(req.body.order);
    order.geometry = geoData.body.features[0].geometry;
    ...
    order.save((error, data) => {
        ...
        const emailData = {
            to: 'don82418@gmail.com', // admin
            from: 'bjongo1@walla.com',
            ...
            <h2>Total products: ${order.product.length}</h2>
...
exports.remove = (req, res) => {
    Order.findByIdAndRemove({_id: req.body.orderId},
        (err, order) => {
            if (err) {
                return res.status(400).json({
                    error: errorHandler(err)
